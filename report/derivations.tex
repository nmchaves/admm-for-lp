...
First, we evaluated how sensitive the ADMM LP algorithms are to the $\beta$ parameter in the augmented Lagrangian. For each algorithm, we recorded the number of iterations required to converge as a function of $\beta$. We did not apply block splitting.

The results are shown in Figure {\color{red}TODO!!}. We observe that all of the ADMM approaches are relatively insensitive to $\beta$. For the rest of our experiments, we fixed $\beta=0.5$. Choosing a different value of $\beta$ should not have much effect on the rest of our results.
...
\begin{figure*}[h]
	...
	\includegraphics[width=\textwidth]{../figures/struct_prob.png}
...
\begin{figure*}[h]
	\caption{Structured Matrix. No Preconditioning, No Random Permutation}
...
\begin{figure*}[h]
	\caption{Structured Matrix. Random Permutation, No Preconditioning}
	\includegraphics[width=\textwidth]{../figures/struct_noprecond_rndperm.png}
	\label{fig:struct_nop_r}
\end{figure*}

\begin{figure*}[h]
	\caption{Structured Matrix. Preconditioning, No Random Permutation}
